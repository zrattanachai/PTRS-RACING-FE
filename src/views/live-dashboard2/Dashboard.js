...
const ALPHA = 1;  // Smoothing factor (0 < ALPHA < 1)
let smoothSpeed = {};  // Store smoothed speed per WebSocket connection
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const handleMessage = (url, message) => {
      ...
      // let rawSpeed = parsedData[3];
      // let rawSpeed = Math.random() * 100;666
      let rawSpeed = (Math.round(parsedData[3]) + 0.1) + Math.random() * 0.8;

      // Initialize smoothSpeed for the URL if it doesn't exist
      if (!smoothSpeed[url]) {
        smoothSpeed[url] = rawSpeed;
      }

      // Apply the EMA formula to smooth the speed data
      smoothSpeed[url] = ALPHA * rawSpeed + (1 - ALPHA) * smoothSpeed[url];
      
      // Update the smoothed speed in parsedData[3]
